...
function(input, output) {
  observe({
    hide(id = "controlbar-toggle")
    addClass(selector = "awesome-checkbox", class = "inline")
  })
  ...
  output$bigger_station <- renderbs4ValueBox({
  ...
  output$n_validation <- renderbs4ValueBox({
  ...
  output$indic_validation <- renderbs4ValueBox({
  ...
  output$indic_validation2 <- renderbs4ValueBox({
    vald_ligne <- nvald_lign_jour_catj()
    if (input$choix_ligne == "ALL") {
    ...
    if (input$choix_ligne == "ALL") {
      NVALDTOT_STAT_CATJOUR_LIGNE %>%
        ...
        addCircleMarkers(
          ...
          lat = ~stop_lat,
          popup = ~ paste0(
            NOM_ARRET, "<br/>", "<i>", "Nombre de validation : ",
      ...
      NVALDTOT_STAT_CATJOUR_LIGNE %>%
        ...
        addPolylines(
          data = plan_metro %>% filter(indice_lig == input$choix_ligne),
          color = ~hexcolor,
          weight = 2,
          opacity = 1
        ) %>%
